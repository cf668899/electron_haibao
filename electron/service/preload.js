...
class PreloadService extends Service {
  ...
  savePreload(data){
    ...
    if(data.type == 'Telegram'){
        let filePath = path.join(Ps.getHomeDir(), 'Telegram.js')
                ...
                if (inputMessages) {
                    ...
                    if (content != '') {
                        if (translatorMap[content]) {
                            ...
                            rest = await window.electron.ipcRenderer.invoke('controller.translator.deepl', { 'translate': newTranslate, 'texts': [content] })
                        }
                    }
