...
class PreloadService extends Service {
    ...
    // 监听消息
    window.electron.ipcRenderer.on('translateOpen', (event, data) => {
            ...
            if (inputBoxs.length > 0) {
                ...
                if (inputs.length) {
                    ...
                    if (inputMessages) {
                        ...
                        let content = inputMessages[0].textContent
                        if (content != '') {
                            if (translatorMap[content]) {
                                rest = {
                                    status: 200,
                                    data: {
                                        translations: [
                                            {
                                                text: translatorMap[content]
                                            }
                                        ]
                                    }
                                }
                            } else {
                                rest = await window.electron.ipcRenderer.invoke('controller.translator.' + translate.type, { 'translate': newTranslate, 'texts': [content] })
                            }
                        }
